...
function ajaxUpdateConfig(id , value) {
	...
	$.ajax({
		...
		success: function(response) {
			...
			try {
				response = $.parseJSON(response);
			} catch(e) { alert('Une erreur s\'est produite (2)'); return false; }
			if (response.type != 'OK' && response.type != 'ERR') { alert('Une erreur s\'est produite (3)'); return false; }
			if (response.type == 'ERR') { alert(response.msg + ' (4)'); return false; }
		...
		error: function() {
			// Decrement when ajax call ends
			window.ajaxUpdates--;
			alert('Une erreur s\'est produite (1)'); return false;
		}
	});
}
$(function() {
	$('.config-auto-update').not('editor_html').each(function(i, v) {
		...
		if ($this.data('val') == $this.val())
			return;
		ajaxUpdateConfig($this.data('item'), $this.val());
		$this.data('val', $this.val());
	});
});
$(window).load(function() {
	for (i=0; i<$.summernote.length; i++) {
		...
		editor.on('blur', function(e) {
			...
			ajaxUpdateConfig($element.data('item'), $(this).summernote('code'));
			$element.data('val', $(this).summernote('code'));
		});
	}
});
...
var initPlugins = function(context) {
	...
	$('.calendar-cours', context).each(function(){
		...
		$this.fullCalendar({
			...
			weekNumbers: true,
			minTime: $this.data('mintime') ? $this.data('mintime') : '07:00',
...
$(document).ready(function() {
	$('.gen-rows').each(function(){
		...
		$btnAdd.click(function(e) {
			...
			$container.append($row).trigger('gen-rows-changed').trigger('gen-row-added', {row: $row});
		}).trigger('click');
	});
});
...
$('.post-detail').on('click', function(e) {
	...
	ajax('get', {
		...
		$('.scroller-content').slimScroll({ scrollTo : '0px' });
	});
});
...
if($('#quiz').length > 0) {
	  ...
	  question_container.bind('user-select-change', function() {
		...
		for (var i = 0; i < self.questions.length; i++) {
		 ...
		 if(self.questions[current_question_index].user_choice_index === self.questions[current_question_index].correct_choice_index) {
				$('input[name=choices]:checked + label').css({"background-color" : "green","color" : "#fff",});
		  } else {
			   $('input[name=choices]:checked + label').css({"background-color" : "#e13034"});
		  }
		  
		if (current_question_index < self.questions.length - 1) {
		  current_question_index++;
		  
		  setTimeout(function(){ change_question(); }, 2000);
		  
		} else {
			
			 setTimeout(function(){ $("#submit-button").trigger("click"); }, 2000);
	...
	if(!this.img_src)
	$('p', question_string_h2).css({"position" : "absolute",
		"top" : "130px",
		"top" : "130px",
		"left" : "0",
		"right" : "130px",
  ...
  $(document).ready(function() {
    ...
    $('#taches_calendar').fullCalendar({
      ...
      views: {
        ...
        listWeek: { buttonText: 'Calendrier hebdomadaire' }
      },
    ...
    function cb(start, end) {
        ...
        $('.dash-periode').val(start.format('YYYY-M-DD') + ' - ' + end.format('YYYY-M-DD'));
		 ...
		 ajax('get', {
			...
			setTimeout(function(){ $.sparkline_display_visible(); }, 200);
		});
    }
...
var width = $(this).parent().width();
	$(".fiche-eleve .slider_tab").css({"left":+ position.left,"width":width});
});
...
$(".fiche-eleve .slider_tab").css({"left":+ actPosition.left,"width": actWidth});
}
