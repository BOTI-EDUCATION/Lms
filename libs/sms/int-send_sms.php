...
require_once "nexmo-api.php";

//require_once dirname(__FILE__) . "/../primotexto-api-php/baseManager.class.php";

//authenticationManager::setApiKey(Config::get('sms-apiKey'));

//function sendSms($message, $phoneNumber, $compainName = "Entretien planifie"){
//    $sms = new Sms;
//    $sms->type = 'notification';
//    $sms->number = $phoneNumber;
//    $sms->message = $message;
//    $sms->sender = Config::get('sms-sender');
//    var_dump("SMS info", $sms);
//
////    $sms->campaignName = $compainName;
//    return messagesManager::messagesSend($sms);
//}

function checkMessageStatus($messageID, $sendCallback, $tries = 0) {
//    if($tries === 3) return;
//
//    var_dump("Snapshot ID", $messageID);
//    // Status by snapshotId
//    $msg = new Query;
//    $msg->type = 'notification';
//    $msg->snapshotId = $messageID;
//    $result = messagesManager::messagesStatus($msg);
//
//    $resultArray = json_decode($result, true);
//
//    if($resultArray["state"] != "delivered"){
//        sleep(10);
//        checkMessageStatus($messageID, $sendCallback);
//    }
//
//    var_dump("Check status", $result);
}
...
//*/
////function sendSevenBitSMS ($seven_bit_msg, $msisdn)
////{
////  $username = Config::get('sms-username');
////  $password = Config::get('sms-password');
////
...
//  $url = Config::get('sms-url');
//  $port = Config::get('sms-port');
//  $post_body = unicode_sms($username, $password, $message, $phoneNumber);
//
//  return send_message($post_body, $url, $port);
//}
//
///*
...
////}
//
//function checkMessageStatus($batchID, $sendCallback, $tries = 0){
//  if($tries === 3) return;
//
//  var_dump("Tries", $tries);
//
//  $username = Config::get('sms-username');
//  $password = Config::get('sms-password');
//  $port = Config::get('sms-port');
//
//  $url = Config::get("sms-check-url");
//  $url = str_replace(":username", $username, $url);
//  $url = str_replace(":password", $password, $url);
//  $url = str_replace(":batch_id", $batchID, $url);
//
//  $ch = curl_init( );
...
//  var_dump("CURL INFO", $curl_info);
//
//  if($curl_info[ 'http_code' ] == 200){
//    $api_result = explode( '|', $response_string );
//    var_dump($api_result);
//    $status_code = (int)$api_result[0];
//    if($status_code != 11){
//      sleep(5);
//      $sendCallback();
//      checkMessageStatus($batchID, $sendCallback, ++$tries);
//    }
//
//  }
//
