...
function xhr(link, method, data, success = null, error = null, complete = null) {
    $.ajax({
        url: link ? link : app.url.base + "ajax",
        method: method,
        data: data,
...
$(document).on('click', '.save-personal-data', function (e) {
    ...
    formData.append('matricule', $('#' + id + '-matricule').val())
    formData.append('massar', $('#' + id + '-massar').val())
    formData.append('nom', $('#' + id + '-nom').val())
    formData.append('prenom', $('#' + id + '-prenom').val())
    formData.append('dob', $('#' + id + '-dob').val())
    formData.append('address', $('#' + id + '-address').val())
    formData.append('sexe', $('input[name=' + id + '-sexe]:checked').val())

    fatherId = $_this.data('father')
    if (fatherId) {
        formData.append('father', fatherId)
    }
    formData.append(id + '-father-nom', $('#' + id + '-father-nom').val())
    formData.append(id + '-father-prenom', $('#' + id + '-father-prenom').val())
    formData.append(id + '-father-email', $('#' + id + '-father-email').val())
    formData.append(id + '-father-tel', $('#' + id + '-father-tel').val())

    motherId = $_this.data('mother')
    if (motherId) {
        formData.append('mother', motherId)
    }
    formData.append(id + '-mother-nom', $('#' + id + '-mother-nom').val())
    formData.append(id + '-mother-prenom', $('#' + id + '-mother-prenom').val())
    formData.append(id + '-mother-email', $('#' + id + '-mother-email').val())
    formData.append(id + '-mother-tel', $('#' + id + '-mother-tel').val())
    ...
    $.ajax({
        ...
        swal({
            title: 'Information bien mise à jour.',
            icon: 'success',
            timer: 4000,
        })
    })
...
$(document).on("click", ".download_files", function (e) {
    ...
    for (var n = 0; n < files.length; n++) {
        ...
        temporaryDownloadLink.setAttribute('download', files[i].slice(files[i].lastIndexOf("/") + 1));
...
$(document).on("change", ".submit_form", function (e) {
...
$(document).on("change", ".toggle-checkbox", function (e) {
    ...
    if (_this.data("trigger-change")) {
        $(_this.data("target")).trigger("change");
    }
});
...
$('.xhr-input-search').on($('.xhr-input-search').data('event') || 'keyup', function () {
    _this = $(this);


    xhr(_this.data('search-url'), 'GET', {
        [_this.attr('name')]: _this.val()
    }, function (res) {
        if (res) {
            $(_this.data('res-to')).html(res);
            $(_this.data('res-to')).show();

        } else {
            $(_this.data('res-to')).html('');
            $(_this.data('res-to')).hide();
...
// params
$(function () {
    $(".params-auto-update-v2").on("change", function (e) {
        $this = $(this);
        value = getValueOfInput($this);
        ajaxUpdateConfig(app.url.base + "/admin/settings/params", $this.data("item"), value);
    });
...
var initPlugins = function (context) {
    ...
    var _initDataTable = function (selector, paging = true, options) {
    ...
    $.each($('.feature_datatable', context), function (ind, element) {

        paging = $(this).data('paging');
        $table = _initDataTable(element, paging)
    ...
    $(".select2", context).each(function () {
        ...
        $this.select2({
            ...
            minimumResultsForSearch: $this.data("show-search") || $this.find("option").length > 6 ?
                0 : Infinity,
        });
    });
    ...
    $("#facture_gloabl_pdf").on("click", function () {
        ...
        $.ajax({
            ...
            url: _this.data("action"),
            data: {
                op: "facture_global_increment_nemuro"
            },
        });
    ...
    function readURL(input, previewEle) {
...
var calcEncaissementTotalMontant = function (ele) {
    ...
    // avoirs 
    $("#form-avoirs > option").prop("disabled", false);
    $("#form-avoirs > option").each(function () {
        const option = $(this);

        console.log(option.data('amount'));

        if (totalMontants > option.data('amount')) {
            option.prop("disabled", true);
        };

    });
    $("#form-avoirs").select2().trigger('change');
    $(".js-total-montants").text(totalMontants);
};
...
// click event
$(".check_montant").on("click", function (e) {
    setTimeout(function () {
    ...
    (function () {
        ...
        function toggleOverlay() {
            if (classie.has(overlayDeoubche, "open")) {
                ...
                if (support.transitions) {
                    overlayDeoubche.addEventListener(
                        transEndEventName,
                        onEndTransitionFn
                    );
                } else {
    ...
    $this.mediumInsert({
        ...
        addons: {
            ...
            images: {
                ...
                deleteMethod: "DELETE",
                deleteScript: app.url.base + "upload-editor.php",
                ...
                fileUploadOptions: {
                    ...
                    url: app.url.base + "upload-editor.php",
                    acceptFileTypes: /(\.|\/)(gif|jpe?g|png|m4a|ogv|mp4|m4v|webm)$/i, // (regexp) Regexp of accepted file types
                ...
                uploadCompleted: function ($el, data) {
                    ...
                    if (file.type == 'video/mp4') {
                        $el.find('img').css('position', 'absolute').css('opacity', 0).css('width', '-webkit-fill-available').css('height', '90px');
                        $el.append('<iframe src="' + file.url + '" style="width: -webkit-fill-available;height: -webkit-fill-available;" allowfullscreen="" scrolling="no" allow="accelerometer *; clipboard-write *; encrypted-media *; gyroscope *; picture-in-picture *;" class="medium-insert-image-active"></iframe>');
                        setTimeout(function () {
                            $el.find('iframe').contents().find('video').get(0).pause();
                        }, 500);
                        // var innerDoc = $el.find('iframe').contentDocument || $el.find('iframe').contentWindow.document;
                    }
                },
            },
        },
...
//   if (checked) $("#periode-range"target).prop("disabled", false);
...
//   var checked = $(this).is(":checked");
//   if (checked)
//     $($(this).data('ele')).prop('disabled', false);
//   else
//     $($(this).data('ele')).prop('disabled', true);
...
$(document).on("click", ".get-modal-view", function (e) {
    e.preventDefault();
    _this = $(this);
    xhr(_this.data('link'), 'GET', {}, function (res) {
        var dialog = bootbox.dialog({
            title: "",
            size: "large",
            message: res,
...
$(document).ready(function () {
    ...
    if ($(".realtime-content").length > 0) {
        $(".realtime-content").each(function (index, ele) {

            _this = $(ele);
            (function fecthingContent() {
                xhr($(ele).data('link'),
                    "get", {},
                    function (data) {
                        $(ele).html(JSON.parse(data).data.html);
                        setTimeout(fecthingContent, 2000); // function refers to itself
                    },
                );
            })();
...
$('.docs-edit').on('click', function () {
    ...
    container.find('#docs_pk').attr('name', 'id').val(_this.data('id'));
    container.find('select[name=type]').val(_this.data('type')).trigger('change');
    container.find('textarea[name=details]').val(_this.data('details'));
...
$(document).on("click", ".toggle_element", function () {
    _this = $(this);
    console.log(_this.data("toggle"));
    const togle_classe = _this.data("toggle");
    $(togle_classe).toggle();
});

$(document).on("click", ".undisable_element", function () {
    _this = $(this);
    if (_this.is(':checked')) {
        $(_this.data("toggle")).prop("disabled", false);
    } else {
        $(_this.data("toggle")).prop("disabled", true);
    }
});

$(document).on("click", ".disable_element", function () {
    _this = $(this);
    if (_this.is(':checked')) {
        $(_this.data("toggle")).prop("disabled", true);
    } else {
...
$(".edit-vehicule").on("click", function () {
    ...
    container
        .find('input[name="numeroInterne"]')
        .val(_this.data("numerointerne"));
    container
...
$(document).on("change", ".filtre_classe", function (e) {
    ...
    $.ajax({
        ...
        $(_this.data("classe")).html("<option value=''> Classes </option>");
        const data = JSON.parse(res);
        data.forEach(function (value, index) {
...
$(document).on("change", ".fill_another_select", function (event) {
    ...
    const dataName = _this.data("name");
    const dataVal = _this.val();
    const dataNiveau = _this.data("niveau") ?
        $(_this.data("niveau")).val() :
        null;
...
$(".update_competence").on("click", function (event) {
    ...
    const sem = _this.data("semester").toString();
    const compSuper = "#update_comp_super";
    const idInput = "#update_comp_id";
    const titleInput = "#update_title";
    const descInput = "#update_desc";
    const s1 = "#modal-semester-1";
    const s2 = "#modal-semester-2";
    $(idInput).val(id);
    $(titleInput).val(title);
    $(descInput).val(desc);

    $(s1).prop("checked", sem.includes("1") ? true : false);
    $(s2).prop("checked", sem.includes("2") ? true : false);
    ...
    $(".select3").each(function () {
        ...
        $this.select2({
            ...
            templateResult: function (data) {
                ...
                return $(data.element).data("html") || $(data.element).html();
            },
...
$(document).on("click", ".slidebars", function (event) {
    _this = $(this);
...
$(window).load(function () {
    var divWidth = $(".wide-table").width();
    $(".responsive-div").width(divWidth + 5);
});
...
$(".responsive-wrapper").scroll(function () {
    if (scrolled) {
        ...
        scrolled = true;
        $(".table-wrapper").scrollLeft($(".responsive-wrapper").scrollLeft());
    }
});
...
$(".table-wrapper").scroll(function () {
    if (scrolled) {
        ...
        scrolled = true;
        $(".responsive-wrapper").scrollLeft($(".table-wrapper").scrollLeft());
    }
});
...
$(".nav-arrow").click(function () {
    console.log("fired");
    _this = $(this);
    console.log($(_this.data("parent")));
    $(_this.data("parent")).addClass("hidden");
    $(_this.data("uncle")).removeClass("hidden");
});

$(document).on("change", ".toggle-other-checkbox", function (e) {
    _this = $(this);
    $(_this.data("target")).prop("checked", _this.is(":checked"));
});

function fillModal(_this, data) {
    const modal = _this.data("modal");

    if ($("body").hasClass("modal-open")) {
        $(modal).modal("hide");
        if (_this.data("reload")) location.reload();
    } else {
        $(_this.data("panel")).html(data.html);
        $(".data-table").DataTable();
        $(modal).modal("show");
    }
}

$(document).on('change', '.toggle-inputs', function (event) {
    _this = $(this)
    allGroups = _this.data('hide')
    targetGroup = _this.data('show')
    addVal = _this.data('addval')
    disable = _this.data('disable')
    require = _this.data('require')
    empty = _this.data('empty')
    inputs = _this.data('inputs')

    if (addVal) targetGroup += _this.val()

    $(allGroups).addClass('hidden')
    $(targetGroup).removeClass('hidden')

    if (disable) {
        $(allGroups).find(inputs).prop('disabled', true)
        $(targetGroup).find(inputs).prop('disabled', false)
    }

    if (require) {
        $(allGroups).find(inputs).prop('required', false)
        $(targetGroup).find(inputs).prop('required', true)
    }

    if (empty) {
        $(allGroups).find(inputs).val('')
    }
})